...
export class EnhancedBlogGenerationService extends OpenAIService {
  ...
  generateTweetCardHTML(tweet) {
    ...
    return `<div class="tweet-card" data-tweet-id="${tweetId}" style="border: 1px solid #e1e8ed; border-radius: 12px; padding: 16px; margin: 24px auto; background: #f8f9fa; width: 90%; max-width: 90%; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;"><div style="display: flex; align-items: center; margin-bottom: 12px;"><img src="${authorAvatar}" alt="${authorName}" style="width: 48px; height: 48px; border-radius: 50%; margin-right: 12px; flex-shrink: 0;" /><div style="flex: 1; min-width: 0;"><div style="display: flex; align-items: center;"><span style="font-weight: 700; color: #14171a; font-size: 15px;">${authorName}</span>${verifiedBadge}</div><span style="color: #657786; font-size: 15px;">@${authorHandle}</span></div><a href="${tweetUrl}" target="_blank" rel="noopener noreferrer" style="color: #1DA1F2; text-decoration: none; flex-shrink: 0;"><svg style="width: 20px; height: 20px; fill: #1DA1F2;" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a></div><p style="color: #14171a; font-size: 15px; line-height: 1.5; margin: 0 0 12px 0; white-space: pre-wrap; word-wrap: break-word;">${tweetText}</p><div style="display: flex; align-items: center; gap: 20px; color: #657786; font-size: 13px; padding-top: 12px; border-top: 1px solid #e1e8ed;"><span>${createdAt}</span><span style="display: flex; align-items: center; gap: 6px;"><span>❤️</span><span>${likes}</span></span><span style="display: flex; align-items: center; gap: 6px;"><span>🔁</span><span>${retweets}</span></span></div></div>`;
  }
