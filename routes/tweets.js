/**
 * Tweet search API routes. POST /api/v1/tweets/search-for-topic-stream for streaming tweet search.
 * Body: { topic, businessInfo, maxTweets? }
  ...
  try {
    ...
    setImmediate(() => {
      enhancedBlogGenerationService
        .searchForTopicStream(topic, businessInfo, maxTweets, connectionId)
        .catch((err) => console.error('tweets search-for-topic-stream error:', err));
    });

    res.status(200).json({ connectionId, streamUrl });
  } catch (error) {
    console.error('tweets search-for-topic-stream endpoint error:', error);
    res.status(500).json({
      success: false,
