/**
 * YouTube video search API routes. POST /api/v1/youtube-videos/search-for-topic-stream for streaming video search.
 * Body: { topic, businessInfo, maxVideos? }
...
router.post('/search-for-topic-stream', async (req, res) => {
  try {
    ...
    const { topic, businessInfo, maxVideos = 5 } = req.body;
    ...
    setImmediate(() => {
      enhancedBlogGenerationService
        .searchForTopicStreamYouTube(topic, businessInfo, maxVideos, connectionId)
        .catch((err) =>
          console.error('youtube-videos search-for-topic-stream error:', err)
        );
    });

    res.status(200).json({ connectionId, streamUrl });
  } catch (error) {
    console.error('youtube-videos search-for-topic-stream endpoint error:', error);
    res.status(500).json({
      success: false,
