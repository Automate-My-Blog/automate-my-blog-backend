# Trending topics stream — frontend hand-off

Streaming trending topic generation so the UI can show topics (and images) as they're ready instead of waiting for the full response. Replaces the non-streaming `POST /api/trending-topics`. Same pattern as **topics** (`POST /api/v1/topics/generate-stream`), **audiences** (`POST /api/v1/audiences/generate-stream`), and **tweets** (`POST /api/v1/tweets/search-for-topic-stream`).
...
- **Method:** `POST`
- **URL:** `${API_BASE}/api/v1/trending-topics/stream` (or `${API_BASE}/api/v1/topics/generate-stream`)
- **Auth:** Either:
...
1. User triggers "Generate trending topics" (e.g. from analysis or topic discovery).
2. `POST /api/v1/trending-topics/stream` with `businessType`, `targetAudience`, `contentFocus`.
3. On 200, open `EventSource(streamUrl)` (or the built URL with `?token=` or `?sessionId=`).
...
7. On **complete**: you have the full `topics` array; close EventSource and treat as done.
8. On **error**: close EventSource, show `data.error` to the user.
...
## 6. Same pattern as topics/audiences/tweets

If you already have code for topics, audiences, or tweet search streaming:

- **Start:** `POST /api/v1/trending-topics/stream` (or `POST /api/v1/topics/generate-stream`) → returns `connectionId` and `streamUrl`.
- **Listen:** Same `GET /api/v1/stream/:connectionId?token=...` or `?sessionId=...` endpoint; only the event names and payloads differ.
- **Trending topics events:** `topic-complete`, `topic-image-start`, `topic-image-complete`, `complete`, `error`.
