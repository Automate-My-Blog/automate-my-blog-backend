/**
 * Unit tests: Tweet search stream route POST /api/v1/tweets/search-for-topic-stream.
 */
...
vi.mock('../../services/enhanced-blog-generation.js', () => ({
  default: {
    searchForTopicStream: vi.fn().mockResolvedValue(undefined)
  }
}));

import tweetRoutes from '../../routes/tweets.js';

describe('tweets search-for-topic-stream', () => {
  let app;
  ...
  beforeEach(() => {
    ...
    // Simulate optionalAuthMiddleware: set req.user when Bearer token present
  ...
  it('returns 401 when no auth (no JWT, no x-session-id)', async () => {
    const res = await request(app)
      .post('/api/v1/tweets/search-for-topic-stream')
      .send({
