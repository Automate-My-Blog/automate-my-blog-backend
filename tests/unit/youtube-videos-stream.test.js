/**
 * Unit tests: YouTube video search stream route POST /api/v1/youtube-videos/search-for-topic-stream.
 */
...
vi.mock('../../services/enhanced-blog-generation.js', () => ({
  default: {
    searchForTopicStreamYouTube: vi.fn().mockResolvedValue(undefined)
  }
}));

import youtubeVideosRoutes from '../../routes/youtube-videos.js';

describe('youtube-videos search-for-topic-stream', () => {
  let app;
  ...
  beforeEach(() => {
    ...
    app.use('/api/v1/youtube-videos', youtubeVideosRoutes);
  });

  it('returns 401 when no auth (no JWT, no x-session-id)', async () => {
    const res = await request(app)
      .post('/api/v1/youtube-videos/search-for-topic-stream')
      .send({
  ...
  it('passes maxVideos to service when provided', async () => {
    ...
    expect(enhancedBlogGenerationService.searchForTopicStreamYouTube).toHaveBeenCalledWith(
      { title: 'Test' },
  ...
  it('uses default maxVideos 5 when not provided', async () => {
    const { default: enhancedBlogGenerationService } = await import(
      '../../services/enhanced-blog-generation.js'
    );
    await request(app)
