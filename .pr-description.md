...
**Website analysis speed and cache.** Research is batched (one call for brand, competitors, keywords) instead of several sequential calls. Results are cached by URL for 30 days so repeat analyses of the same site are fast. Cache is keyed by URL only.

**Content generation speed.** After the blog body is ready, visual suggestions and SEO analysis run in parallel. The existing-audiences query is overlapped with the analyze step where possible. Same API shape; jobs just finish sooner.
