...
export function buildAuthSetCookieHeaders(accessToken, refreshToken) {
  ...
  // Quote values per RFC 6265 so commas/semicolons in token don't break parsing
  const av = String(accessToken).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  const rv = String(refreshToken).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  return [
    `${COOKIE_NAMES.access}="${av}"; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=${ACCESS_MAX_AGE}${domainPart}`,
    `${COOKIE_NAMES.refresh}="${rv}"; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=${REFRESH_MAX_AGE}${domainPart}`
  ];
}
